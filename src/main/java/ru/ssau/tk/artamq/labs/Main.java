...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Function lnFromExp = Functions.composition(exp, ln);
        TabulatedFunction tabulatedLnFromExp = TabulatedFunctions.tabulate(lnFromExp, 0, 10, 11);

        System.out.println("Табулированный ln(exp^x): ");
        System.out.printf("Количество точек: %s\n", tabulatedLnFromExp.getPointsCount());
        tabulatedLnFromExp.traverse();

        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("tabulatedLnFromExp.ser"))) {
            oos.writeObject(tabulatedLnFromExp);
            System.out.println("Объект сериализован");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("tabulatedLnFromExp.ser"))) {
            TabulatedFunction readTabulatedLnFromExp = (TabulatedFunction) ois.readObject();
